    ...
    <AppLayout title="Tableau de bord">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Voici un aperçu de l'activité de la plateforme DGPSN.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-4">
            ...
            <div class="lg:col-span-3 bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-sm font-semibold text-gray-800">Évolution des demandes</h2>
                    <select class="text-xs border border-gray-200 rounded-lg px-2 py-1 text-gray-600 focus:outline-none focus:ring-1 focus:ring-green-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div class="lg:col-span-3 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <h2 class="text-sm font-semibold text-gray-800">Dernières demandes</h2>
                    <Link href="/demandes" class="text-xs text-green-700 font-medium hover:underline">Voir tout</Link>
                </div>
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-xs text-gray-500 bg-gray-50 border-b border-gray-100">
                            ...
                            <th class="text-left px-3 py-3 font-medium">Citoyen</th>
                            <th class="text-left px-3 py-3 font-medium">Type d'aide</th>
                            <th class="text-left px-3 py-3 font-medium">Statut</th>
                            <th class="text-left px-3 py-3 font-medium">Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr
                            v-for="d in dernieresDemandes"
                            :key="d.id"
                            class="border-b border-gray-50 hover:bg-gray-50 transition-colors"
                        >
                            <td class="px-5 py-3 font-mono text-xs text-gray-700">{{ d.reference }}</td>
                            <td class="px-3 py-3 text-gray-800 font-medium">{{ d.citoyen }}</td>
                            <td class="px-3 py-3 text-gray-600">{{ d.type }}</td>
                            <td class="px-3 py-3">
                                <StatusBadge :status="d.statut" />
                            </td>
                            <td class="px-3 py-3 text-gray-500 text-xs">{{ d.date }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <!-- Activity feed -->
            <div class="lg:col-span-2 bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                <h2 class="text-sm font-semibold text-gray-800 mb-4">Activité récente</h2>
                <div class="space-y-4">
                    <ActivityItem
                        v-for="item in activiteRecente"
                        :key="item.id"
                        :icon="item.icon"
                        :title="item.titre"
